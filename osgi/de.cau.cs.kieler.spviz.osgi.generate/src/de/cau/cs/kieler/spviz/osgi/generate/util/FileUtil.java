...
public class FileUtil {
     ...
     *
     * @param entry
     *            the jar entry with everything included in the jar file
     * @param jarFile
     *            needed to create the input stream
     * @param tempFilePath
     ...
     *            the bundle path
     * @param manifest
     *            the manifest for obtaining the name of the jar
     * @throws IOException
     *             in case the rootPath is wrong
     */
    // CHECKSTYLE IGNORE AbbreviationAsWordInNameCheck FOR NEXT 1 LINES - OSG is what it is called.
    ...
    /**
     * Extracts the OSGI-INF directory from a jar, if it exists, and puts it into the temporary folder.
     *
     * @param rootPath
     *            root of the bundle
     * @param fileWithJarEnding
     *            the jar which the OSGI-INF files are being extracted from
     * @throws IOException
     *             in case there are file issues.
     *
     */
    ...
    private static List<Path> listFilesWithExtensionRecursively(final Path path, final String fileExtension)
        ...
        final Filter<Path> filter =
            filterPath -> filterPath.toString().endsWith(fileExtension) || filterPath.toFile().isDirectory();
